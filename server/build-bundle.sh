#!/usr/bin/env bash
# Vendor the built viewer into the pagoda3 packages for same-origin local serving (Phase 2).
# Builds the SPA (base "/") and copies it into py/src/pagoda3/_viewer (and r/inst/viewer), MINUS the
# demo *.lstar.zarr stores. Run before building a wheel / R package. The _viewer dirs are gitignored
# (regenerated here), so a source checkout falls back to web/dist via pagoda3.bundle.
set -euo pipefail
