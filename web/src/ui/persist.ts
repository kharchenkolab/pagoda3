...
export interface SessionDoc { v: number; store: string; fingerprint?: Fingerprint; currentWS: string; colorBy: string; canvas: any[]; userWS: { name: string; ws: any }[]; annotation?: SerAnnoLayer[]; conversation?: SerConversation; catColors?: Record<string, [number, number, number]>; style?: Record<string, any>; customPalette?: number[][]; results?: any[]; customGeneSets?: any[]; geneFilter?: string[]; }

export function serializeSession(d: { store: string; fingerprint?: Fingerprint; currentWS: string; colorBy: string; canvas: any[]; userWS: { name: string; ws: any }[]; annotation?: SerAnnoLayer[]; conversation?: SerConversation; catColors?: Record<string, [number, number, number]>; style?: Record<string, any>; customPalette?: number[][] | null; results?: any[]; customGeneSets?: any[]; geneFilter?: string[] }): string {
  return JSON.stringify({ v: VERSION, store: d.store, fingerprint: d.fingerprint, currentWS: d.currentWS, colorBy: d.colorBy, canvas: d.canvas, userWS: d.userWS, annotation: d.annotation, conversation: d.conversation, catColors: d.catColors, style: d.style, customPalette: d.customPalette, results: d.results, customGeneSets: d.customGeneSets, geneFilter: d.geneFilter });
}
...
export function parseSession(raw: string | null): SessionDoc | null {
  ...
  try { const o = JSON.parse(raw); if (!o || o.v !== VERSION || !Array.isArray(o.canvas)) return null;
    return { v: o.v, store: String(o.store || ""), fingerprint: parseFingerprint(o.fingerprint), currentWS: String(o.currentWS || ""), colorBy: String(o.colorBy || ""), canvas: o.canvas, userWS: Array.isArray(o.userWS) ? o.userWS : [], annotation: parseAnnotation(o.annotation), conversation: parseConversation(o.conversation), catColors: (o.catColors && typeof o.catColors === "object") ? o.catColors : undefined, style: (o.style && typeof o.style === "object") ? o.style : undefined, customPalette: Array.isArray(o.customPalette) ? o.customPalette : undefined, results: Array.isArray(o.results) ? o.results : undefined, customGeneSets: Array.isArray(o.customGeneSets) ? o.customGeneSets : undefined, geneFilter: Array.isArray(o.geneFilter) ? o.geneFilter.map(String) : undefined };
  } catch { return null; }
}
