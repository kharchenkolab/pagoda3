...
const TOOLS: Tool[] = [
  ...
  { name: "describe_panel", description: "Reflective lookup — WHAT YOU CAN STYLE on a panel. Returns each styleable key (point / label / selection / crosshair / fit / …) with its CURRENT value, default, and valid range, read live from the renderer so it never drifts from what paint honours. Call it before a visual tweak you're unsure how to express (like reading a tool's schema before calling it), then set the keys via update_view({style:{…}}) — or the pointSize/labelSize shortcuts. id = the panel (LAYOUT lists ids); omit for the main embedding.", input_schema: { type: "object", properties: { id: { type: "number", description: "panel id (omit → the main embedding)" } } } },
...
async function execTool(app: App, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    case "describe_panel": {
      const d = app.describePanelStyle(typeof input?.id === "number" ? input.id : undefined);
      if (d.note) return `${d.type}${d.id != null ? ` #${d.id}` : ""}: ${d.note}`;
      ...
      const setHint = `update_view({style:{<family>:{<key>:value}}${d.id != null ? `, stylePanel:${d.id}` : ""}})`;
      return `Styleable knobs for ${d.type}${d.id != null ? ` #${d.id}` : " (main embedding)"} — set via ${setHint}; pointSize/labelSize are px shortcuts:\n${lines.join("\n")}`;
    }
    case "update_view": {
