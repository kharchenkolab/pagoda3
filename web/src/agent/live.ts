...
import { listRecipes, findRecipes, getRecipe } from "../widget/recipes.ts";
...
- CUSTOM WIDGETS (generative UI) are the UI escape hatch: when the user wants a bespoke interactive panel the built-ins don't provide (a custom chart, control, calculator, mini-tool), AUTHOR one. For anything CHART/VIZ/INTERACTION-shaped, LOOK UP recipes first: find_widget_recipe('what you need') → get_widget_recipe each hit. It returns whole widgets to adapt AND inlinable snippets — the plot kit (scales, nice-ticks, canvas point cloud, nearest-point hit-test for point hover/click, colour ramps, SVG axes, binning). That's how you get 'a plotting library' while staying self-contained: pull only the snippets you need and paste them in (don't hand-roll axes/scales/hit-testing from scratch, and never load a CDN). Fall back to get_widget_template (kitchen sink) / read_widget_contract for the base API. Then preview_widget ONCE with the full source (pass a 'probe' to exercise interactive logic) → FIX with edit_widget (small search/replace patches, NOT a whole-source re-emit) until ok:true → save_widget with NO source (reuses the last preview). After saving (or when the user says a widget misbehaves), inspect_widget to CHECK its live use — its rendered text/logs/errors against real data + the current selection — and fix if needed. For a technique the recipes don't cover, fetch_url can pull an external reference, but the widget itself stays self-contained (no CDN). A widget reads/writes the SAME coordination space (selection/colour) and themes automatically. Prefer built-in panels when they fit; author a widget for the genuine long tail.
...
async function execTool(app: App, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    ...
    case "preview_widget": {
      if (input?.source != null) lastWidgetSource = String(input.source);   // omit source to re-preview the current widget without re-emitting it
      if (!lastWidgetSource) return JSON.stringify({ ok: false, error: "no source — pass the full source the first time" });
    ...
    case "edit_widget": {
      if (!lastWidgetSource) return JSON.stringify({ ok: false, error: "no widget yet — preview_widget with the full source first" });
