...
ANNOTATION (the Annotate workspace): reconcile candidate labelings into one clean cell-type annotation. run_annotation adds a SOURCE (e.g. sctype); get_reconciliation reads how sources compare per cluster + where they differ; adopt_source sets the whole working draft to a source's labeling in one step; annotate writes a label onto a cell set in the WORKING draft (last-write-wins, non-destructive). Typical flow: run_annotation → adopt_source the best one → fix the few wrong clusters with annotate. Advise and explain (markers + the confusion matrix) — cross-source string differences are often just vocabulary, not real conflicts. When a labeling does NOT map 1:1 to clusters (a source SPLITS a cluster, or two sources disagree at sub-cluster resolution), reconcile by INTERSECTION rather than per-cluster: annotate the exact disagreeing cells, e.g. A={intersect:[{category:{grouping:"scType",value:"CD14+ monocyte"}},{category:{grouping:"annotation",value:"NK"}}]} labels just the cells scType calls monocyte but the draft calls NK. The confusion matrix's off-diagonal counts are exactly these intersections.
