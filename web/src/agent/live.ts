...
async function execTool(app: App, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    ...
    case "preview_widget": {
      ...
      return JSON.stringify({ ok: r.ok, error: r.error, logs: r.logs.slice(-8), manifest: r.manifest, renderedText: (r.text || "").slice(0, 400), emitted: r.emitted && r.emitted.length ? r.emitted : undefined });
    }
    case "edit_widget": {
      if (input?.from) { const s = recipeSource(String(input.from)); if (!s) return JSON.stringify({ ok: false, error: "no recipe/snippet '" + input.from + "'" }); lastWidgetSource = s; }   // adapt a recipe: seed it, then apply the edits
      ...
      const sim = await resolveSim(app, input);
      const r = await previewWidget(lastWidgetSource, previewHost(app, sim), 6000, input?.probe ? String(input.probe) : undefined);
      return JSON.stringify({ ok: r.ok, error: r.error, logs: r.logs.slice(-8), manifest: r.manifest, renderedText: (r.text || "").slice(0, 400), emitted: r.emitted && r.emitted.length ? r.emitted : undefined, applied: res.applied });
    }
