...
async function execTool(app: App, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    case "describe_panel": {
      ...
      if (d.controls) {   // a WIDGET — its instance-declared header controls (triggerable)
        if (!d.controls.length) return `${head}: ${d.note}`;
        return `${head} — triggerable controls:\n${d.controls.map((c) => `- ${c.id} ("${c.label}")`).join("\n")}\n${d.note}`;
      }
      if (d.params) {     // a built-in with a style descriptor
        ...
        return `Styleable knobs for ${head} — set via update_view({style:{<family>:{<key>:value}}${d.id != null ? `, stylePanel:${d.id}` : ""}}); pointSize/labelSize are px shortcuts:\n${lines.join("\n")}`;
      }
      return `${head}: ${d.note}`;
    }
    case "update_view": {
    ...
    case "save_widget": { const src = String(input?.source || lastWidgetSource); if (!src.trim()) return "save_widget: no source — preview_widget first, then save (omit source to reuse it)"; const { id, updated } = app.addWidgetPanel(src, input?.title); lastWidgetFrom = ""; return updated ? `updated widget panel #${id} in place (same title) — no duplicate added` : `mounted widget panel #${id} on the workbench (a NEW widget starts fresh — pass full \`source\` or \`from:\` a recipe; the previous source is no longer "current")`; }
