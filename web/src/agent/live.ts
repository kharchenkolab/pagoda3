...
async function execTool(app: App, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    ...
    case "show_marker_heatmap": {
      ...
      ag.addRail({ type: "Heatmap", title: `Marker ${dot ? "dotplot" : "heatmap"} · ${grouping}`, cap: `top genes × ${grouping}`, group: grouping, bind: "markers:" + grouping, heatMode: dot ? "dot" : "heat", full: true });
      const mk = await app.ctx.markers(grouping);
      const eg = [...mk.entries()].slice(0, 3).map(([g, r]) => `${g}: ${r.slice(0, 3).map((x) => x.symbol).join("/")}`).join("; ");
      return `added a marker ${dot ? "dotplot (dot size = % of cells expressing)" : "heatmap"} for the ${grouping} grouping (${[...mk.keys()].length} groups) and coloured the embedding by it; e.g. ${eg}`;
    }
    ...
    case "add_panel": {
      ...
      const id = app.addPanel({ type: input.type, title: input.title || input.type, group: grp, heatMode: isHeat && dot ? "dot" : undefined, bind: input.type === "Embedding" ? "embedding:main" : (isHeat ? "markers:" + grp : undefined), view: Object.keys(view).length ? view : undefined });
