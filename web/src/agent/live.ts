...
async function execTool(app: App, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    ...
    case "add_panel": {
      ...
      return `added panel #${id} (${input.type})${view.colorBy ? ` colorBy=${view.colorBy}` : ""}${view.scope ? ` scope=${input.scopeValue}` : ""}${view.embedding ? ` emb=${view.embedding}` : ""}`;
    }
    case "de_between": {
