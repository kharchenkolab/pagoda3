...
export class Agent {
  ...
  async askMock(qraw: string, sc?: Scope | null) {
    ...
    if (!wantsPrograms && /variabl|\bhvg\b|overdispers|most variable/.test(q)) {   // per-gene HVG (the default meaning of "variable/overdispersed genes")
      ...
      this.addRail({ type: "GeneList", title: `Variable genes · ${scope}`, cap: "od (resid)", bind: "hvg:scope", rows: hv.map((h) => ({ symbol: h.symbol, score: h.resid })) }, qraw);
  ...
  // FIRST-RUN greeting — NOT a canned finding (a boot-time data claim like "c0 is enriched in disease" is just noise).
  // Shine the Ask button + a one-line invite; clicking opens the palette's sample queries (nudgePending stays null) and
  // the shine clears on first interaction. The nudgePending/openNudge path is reserved for GENUINE agent findings.
  armIntro() { this.app.setPip("intro"); this.app.toast("Ask me anything about your data", "Run DE, annotate cell types, compose custom views, even build widgets — click Ask (or ⌘K) for examples."); }
