...
export interface EmbeddingStyle {
  ...
  hint: { grow: number; opacity: number; ring: number; dim: number; ringThreshold: number; mode: "adaptive" | "ring" | "lift" | "fill" };
...
export const EMBEDDING_RANGES: Record<string, [number, number]> = {
  ...
  "selection.fillGrow": [0, 24], "selection.fillOpacity": [0, 255], "hint.grow": [0, 24], "hint.opacity": [0, 255], "hint.ring": [0, 6], "hint.dim": [0.05, 1], "hint.ringThreshold": [0, 1e9],
